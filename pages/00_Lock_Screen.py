...
.lock-overlay {position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at center, #0a0a1a 0%, #000 100%); z-index: 9999; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px;}
.lock-title {font-family: 'Orbitron', sans-serif; font-size: 38px; font-weight: 900; background: linear-gradient(90deg, #00FF88, #00AAFF, #AA00FF); -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: glow 2s ease-in-out infinite; text-align: center; margin-bottom: 10px;}
@keyframes glow { 0%,100%{filter: brightness(1);} 50%{filter: brightness(1.5);} }
.login-card {background: #111122; border: 2px solid #00FF88; border-radius: 20px; padding: 30px; width: 450px; box-shadow: 0 0 30px rgba(0,255,136,0.3); margin-top: 20px;}
.animation-box {width: 100%; max-width: 650px; height: 350px; border: 2px solid #00AAFF; border-radius: 15px; margin: 20px 0; overflow: hidden; box-shadow: 0 0 20px rgba(0,170,255,0.4);}
</style>
...
if not st.session_state.logged_in:
    ...
    st.markdown("<p style='color:#00FF88; text-align:center; font-size:14px;'>🛡️ Detecting Bot Attacks | Blocking Spam | Protecting Accounts</p>", unsafe_allow_html=True)
    ...
    with st.container():
        ...
        if result and 'token' in result:
            st.session_state.logged_in = True; st.session_state.user = result.get('userinfo', {'email': 'google_user@gmail.com'}); st.switch_page("pages/01_dashboard.py")
        
        if st.button("🔓 Unlock Dashboard", type="primary", use_container_width=True):
            if puzzle == "8" and email and password:
                st.session_state.logged_in = True; st.session_state.user = {'email': email}; st.success("Access Granted!"); time.sleep(1); st.switch_page("pages/01_dashboard.py")
